...
它把一条已经验证过的流程整理成可复现、可审计、可再次执行的工作流：从分享链接或 `book_id` 出发，读取目录、批量获取章节、断点续传、按章节去重、生成 TXT/EPUB、校验产物，再把 `manifest.json` 和 EPUB 交给服务器做幂等导入。

## 这次发布解决了什么

- 把“脚本 + 手工经验”整理成了清晰的端到端流程仓库。
- 把缓存、去重、EPUB 校验、manifest 导入这些容易漏掉的环节固定下来。
- 把中英文 README、架构说明、故障排查和正式报告一起补齐，适合直接复用或二次改造。

## 这次发布包含

- Python 工具：目录导出、EPUB 写入、EPUB 校验、manifest 生成、上传导入示例。
