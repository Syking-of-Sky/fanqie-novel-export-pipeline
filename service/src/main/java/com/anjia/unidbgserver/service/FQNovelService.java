        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                try (GZIPInputStream gzipInputStream = new GZIPInputStream(new ByteArrayInputStream(response.getBody()))) {
                    ...
                    log.error("GZIP 解压失败，响应状态: {}, 响应字节数: {}",
                        response.getStatusCodeValue(), response.getBody() == null ? 0 : response.getBody().length, e);
                }
                ...
                log.error("批量获取章节内容失败 - requested chapter count: {}", countDelimitedIds(itemIds), e);
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            for (int attempt = 0; attempt <= maxAttempts; attempt++) {
                try {
                    ...
                    log.error("批量获取章节内容失败 - requested chapter count: {}",
                        countDelimitedIds(itemIds), e);
                    return FQNovelResponse.error("批量获取章节内容失败: " + e.getMessage());
                }
            }
...
*/
    ...
    public CompletableFuture<FQNovelResponse<List<Map.Entry<String, String>>>> getDecryptedContents(String itemIds, String bookId, boolean download) {
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                log.error("获取解密章节内容失败 - requested chapter count: {}", countDelimitedIds(itemIds), e);
    ...
    private List<String> getItemIdsByChapterPositions(String bookId, List<String> chapterPositions) {
        ...
        try {
            ...
            if (directoryResponse.getCode() != 0 || directoryResponse.getData() == null) {
                log.error("获取书籍目录失败 - response message: {}", directoryResponse.getMessage());
                return itemIds;
            }
                ...
                try {
                    ...
                    if (index >= 0 && index < catalogItems.size()) {
                        ...
                        if (itemId != null && !itemId.trim().isEmpty()) {
                            ...
                            log.warn("章节位置 {} 对应的itemId为空", position);
                        }
                    } else {
                        log.warn("章节位置 {} 超出目录范围，总章节数: {}", position, catalogItems.size());
                    }
                } catch (NumberFormatException e) {
                    log.error("无效的章节位置: {}", positionStr, e);
                }
            }

        } catch (Exception e) {
            log.error("获取章节itemIds失败 - requested position count: {}", chapterPositions.size(), e);
        }

        return itemIds;
    }

    private int countDelimitedIds(String ids) {
    ...
    private int chapterCount(FQBatchChapterRequest request) {
        if (request == null) {
            return 0;
        }
        if (request.getChapterIds() != null && !request.getChapterIds().isEmpty()) {
            return request.getChapterIds().size();
        }
        List<String> parsed = parseChapterRange(request.getChapterRange());
        return parsed.size();
    }
}
