...
public class FQRegisterKeyService {
    ...
    public void initialize() {
        if (!fqApiProperties.isEnabled()) {
            log.info("FQ upstream calls are disabled; registerkey initialization skipped");
            return;
        }
        ...
        try {
            ...
            if (response != null && response.getData() != null) {
                ...
                log.debug("初始registerkey获取成功，keyver: {}", keyver);
            } else {
    ...
    private FqRegisterKeyResponse fetchRegisterKey() throws Exception {
        ...
        // 创建请求载荷
        FqRegisterKeyPayload payload = new FqRegisterKeyPayload(var, fqApiProperties.getRegistrationKey());
        HttpEntity<FqRegisterKeyPayload> entity = new HttpEntity<>(payload, httpHeaders);

        log.debug("发送registerkey请求到已配置的上游端点");
        log.debug("请求时间戳: {}", currentTime);
        log.debug("签名请求头数量: {}", httpHeaders.size());
        log.debug("请求载荷已生成，keyver={}", payload.getKeyver());
    ...
    public String getDecryptionKey(Long requiredKeyver) throws Exception {
        ...
        return registerKeyResponse.getData().getDecryptedKey(fqApiProperties.getRegistrationKey());
    }
