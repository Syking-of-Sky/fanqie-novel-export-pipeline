...
public class FQSearchController {
    ...
    public CompletableFuture<FQNovelResponse<FQSearchResponse>> searchBooksGet(
        ...
        if (log.isDebugEnabled()) {
            log.debug("搜索书籍请求(GET) - tabType={}, offset={}, count={}, searchIdProvided={}",
                tabType, offset, count, searchId != null && !searchId.trim().isEmpty());
        }
    ...
    public CompletableFuture<FQNovelResponse<FQSearchResponse>> searchBooksPost(
        ...
        if (log.isDebugEnabled()) {
            log.debug("搜索书籍请求(POST) - tabType={}, searchIdProvided={}",
                searchRequest.getTabType(),
                searchRequest.getSearchId() != null && !searchRequest.getSearchId().trim().isEmpty());
        }
    ...
    public CompletableFuture<FQNovelResponse<FQSearchResponse>> quickSearch(
        ...
        if (log.isDebugEnabled()) {
            log.debug("快速搜索请求 - queryLength={}", query == null ? 0 : query.trim().length());
        }
