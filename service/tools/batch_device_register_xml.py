...
SENSITIVE_KEYS = {
    "android_id", "cdid", "clientudid", "device_id", "install_id", "openudid",
    "req_id", "sig_hash", "ipv6_address", "_gen_time", "_rticket", "url",
    "headers", "params", "payload", "device_info", "content", "json", "error",
}


def redact_mapping(value: Any, key: str = "") -> Any:
    """Return a JSON/YAML-safe copy without runtime identifiers or request data."""
    normalized_key = key.lower().replace("-", "_")
    if normalized_key in {item.replace("-", "_") for item in SENSITIVE_KEYS} or any(
        marker in normalized_key
        for marker in (
            "cookie", "token", "secret", "password", "authorization", "uuid",
            "device_id", "install_id", "cdid", "base_url",
        )
    ):
        return "<redacted>"
    if isinstance(value, dict):
        return {name: redact_mapping(item, name) for name, item in value.items()}
...
class EnhancedDeviceRegisterClient:
    ...
    def __init__(self):
        self.base_url = os.getenv("FANQIE_DEVICE_REGISTER_URL", "https://example.invalid/device_register/")
        self.app_info = {
            "display_name": "番茄小说",
            "aid": 1967,
            "channel": "googleplay",
...
def generate_xml_config(device_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        "fq": {
            "api": {
                "base-url": os.getenv("FANQIE_API_BASE_URL", "https://example.invalid"),
...
def save_results(results: List[Dict[str, Any]], xml_configs: List[Dict[str, Any]],
    ...
    """保存结果到 results/；默认只落盘脱敏摘要。"""
    # 确保results目录存在
    os.makedirs("results", exist_ok=True)

    save_full = os.getenv("FANQIE_SAVE_FULL_RESULTS", "0") == "1"
    summary = []
    ...
    summary_file = os.path.join("results", f"device_register_summary_{timestamp}.json")
    with open(summary_file, "w", encoding="utf-8") as f:
        json.dump({"devices": summary, "count": len(summary)}, f, indent=2, ensure_ascii=False)

    safe_configs = [redact_mapping(config) for config in xml_configs]
    ...
    with open(xml_configs_file, "w", encoding="utf-8") as f:
        yaml.dump({"devices": safe_configs}, f, default_flow_style=False, allow_unicode=True)
    ...
    if save_full:
        ...
        print(f"[WARN] 完整结果已按 FANQIE_SAVE_FULL_RESULTS=1 保存到忽略目录: {full_results_file}")
    else:
        print("[INFO] 默认仅保存脱敏摘要；如确需本地完整配置，请设置 FANQIE_SAVE_FULL_RESULTS=1")

    print(f"[INFO] 脱敏摘要已保存到: {summary_file}")
    print(f"[INFO] 脱敏XML配置已保存到: {xml_configs_file}")
    print(f"[INFO] 已生成 {len(xml_configs)} 个配置摘要")

    return summary_file, xml_configs_file
